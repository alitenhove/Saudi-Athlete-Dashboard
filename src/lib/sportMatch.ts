...
function pillarScores(
  ...
  const sprints = cohort
    .map((a) => a.bestSprint30m)
    .filter((v): v is number => v != null);
  const jumps = cohort
    .map((a) => a.bestVerticalJump)
    .filter((v): v is number => v != null);
  const pulls = cohort
    .map((a) => a.bestMidThighPull)
    .filter((v): v is number => v != null);
  ...
  return {
    ...
    endurance:
      athlete.shuttleRun.level != null
        ? percentileRank(
            athlete.shuttleRun.level * 10 +
              (athlete.shuttleRun.shuttlesAchieved ?? 0),
            shuttles,
          )
        : 0.35,
  };
